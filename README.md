<div align="center">

```
██╗  ██╗██████╗ ██╗███████╗██╗  ██╗███╗   ██╗ █████╗     ██╗  ██╗ █████╗ ███╗   ██╗████████╗
██║ ██╔╝██╔══██╗██║██╔════╝██║  ██║████╗  ██║██╔══██╗    ██║ ██╔╝██╔══██╗████╗  ██║╚══██╔══╝
█████╔╝ ██████╔╝██║███████╗███████║██╔██╗ ██║███████║    █████╔╝ ███████║██╔██╗ ██║   ██║   
██╔═██╗ ██╔══██╗██║╚════██║██╔══██║██║╚██╗██║██╔══██║    ██╔═██╗ ██╔══██║██║╚██╗██║   ██║   
██║  ██╗██║  ██║██║███████║██║  ██║██║ ╚████║██║  ██║    ██║  ██╗██║  ██║██║ ╚████║   ██║   
╚═╝  ╚═╝╚═╝  ╚═╝╚═╝╚══════╝╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝  ╚═╝    ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝   ╚═╝   
```

### Data Scientist · ML Enthusiast · Open Source Contributor

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&pause=1000&color=00D9FF&center=true&vCenter=true&width=500&lines=Turning+raw+data+into+real+decisions;Python+%7C+SQL+%7C+Machine+Learning;Building+models+that+actually+matter)](https://git.io/typing-svg)

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/krishna-kant-08752a369)
[![Profile Views](https://komarev.com/ghpvc/?username=KanhaMishra7&style=for-the-badge&color=00D9FF&label=PROFILE+VIEWS)](https://github.com/KanhaMishra7)

</div>

---

## 👨‍💻 About Me

> *"Data is the new oil — but only after it's been refined."*

I'm a **Data Scientist** with a strong foundation in logic and quantitative analysis. I specialize in extracting actionable insights from complex, messy datasets — from raw ingestion to production-ready models.

- 🔭 **Currently:** Advanced Data Analysis & SQL query optimization
- 🌱 **Learning:** Machine Learning algorithms & Generative AI
- 🤝 **Open to:** Open-source collaboration & automated data pipeline projects
- 💬 **Expertise:** Python logic, data cleaning, statistical visualization
- 📍 **Based in:** Greater Noida, India

---

## 🛠️ Tech Stack

### Core Technologies
![Python](https://img.shields.io/badge/Python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54)
![SQL](https://img.shields.io/badge/SQL-CC2927?style=for-the-badge&logo=postgresql&logoColor=white)
![Excel](https://img.shields.io/badge/Excel-217346?style=for-the-badge&logo=microsoftexcel&logoColor=white)

### Data Science & ML
![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy&logoColor=white)
![Scikit-Learn](https://img.shields.io/badge/Scikit--Learn-F7931E?style=for-the-badge&logo=scikit-learn&logoColor=white)
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557c?style=for-the-badge&logo=python&logoColor=white)

### Dev Environment
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=for-the-badge&logo=Jupyter&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05033?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-121011?style=for-the-badge&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white)

---

## 📊 GitHub Stats

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=KanhaMishra7&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true&hide_border=true"/>
<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=KanhaMishra7&layout=compact&langs_count=8&theme=tokyonight&hide_border=true"/>

</div>

<div align="center">

[![GitHub Streak](https://streak-stats.demolab.com?user=KanhaMishra7&theme=tokyonight&hide_border=true)](https://git.io/streak-stats)

</div>

---

## 🚀 Featured Projects

<div align="center">

| Project | Description | Stack |
|--------|-------------|-------|
| 🤖 **Predictive Modeling** | Classification & regression pipelines with cross-validation, hyperparameter tuning, and performance benchmarking | `Python` `Scikit-Learn` `Pandas` |
| 🔍 **Exploratory Data Analysis** | End-to-end EDA framework — automated data profiling, outlier detection, and statistical visualization | `Python` `Pandas` `Matplotlib` `Seaborn` |
| 🗄️ **SQL Query Optimization** | Complex multi-join queries, window functions, and indexing strategies for large-scale business intelligence | `SQL` `PostgreSQL` `Excel` |

</div>

---

## 📈 Activity

<!--START_SECTION:activity-->
[![Krishna's GitHub Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=KanhaMishra7&theme=tokyo-night&hide_border=true)](https://github.com/ashutosh00710/github-readme-activity-graph)
<!--END_SECTION:activity-->

---

## 🧠 What I'm Currently Exploring

```python
krishna = {
    "currently_learning": ["Generative AI", "LLMs", "Deep Learning"],
    "working_on":         ["Advanced EDA pipelines", "SQL optimization frameworks"],
    "looking_to_build":   ["Automated ML pipelines", "Open-source data tools"],
    "ask_me_about":       ["Python", "Data Cleaning", "Statistical Analysis", "SQL"],
    "goal_2026":          "Ship 5 impactful open-source data science projects"
}
```

---

## 🤝 Let's Connect

<div align="center">

I'm always open to interesting conversations, collaborations, and opportunities.

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Krishna%20Kant-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/krishna-kant-08752a369)
[![Email](https://img.shields.io/badge/Email-Reach%20Out-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:your-email@example.com)

---

*"Every dataset tells a story. My job is to find it."*

⭐ **If you find my work useful, consider starring a repo!**

</div>
